...
def send():
    if textReceipt.get(1.0,END)=='\n':
        ...
        def send_msg():
            ...
            auth='izvPKNS4WZZJaimBmDMg7ALsLqyw6TfK1X3gnIQu2awNCz48EenINzFqgjFF'
    ...
    numberLabel=Label(root2,text='Mobile Number',font=('candara',18,'bold underline'),bg='#1c0601',fg='white')
...
# Cakes
        ...
        if e_BlackForest.get() != '0':
            textReceipt.insert(
                END, f'Black Forest Cake:\t\t\t{int(e_BlackForest.get()) * 1000}\n\n')
...
root.geometry('1170x700+0+0')
root.resizable(0, 0)
root.wm_iconbitmap('restaurant-icon.ico')  # for tital icon
root.title('Restaurent Management System created by Avinah')
...
LabelTitle = Label(topframe, text='RESTAURENT MANAGEMENT SYSTEM', font=(
    'candara', 30, 'bold'), fg='white', bg='#0f0300', width=41, pady=5)
LabelTitle.grid(row=0, column=0, padx=5, pady=3)

# frames
menuframe = Frame(root, bd=5, relief=RIDGE, bg='#0a0200', padx=1)
menuframe.pack(side=LEFT, padx=10)

costFrame = Frame(menuframe, bd=3, relief=RIDGE, bg='#0a0200')
costFrame.pack(side=BOTTOM, pady=2)

foodframe = LabelFrame(menuframe, text='Food', font=(
    'candara', 18, 'bold'), bd=3, relief=RIDGE, bg='white', fg='#E14D2A')
...
textBlackForest = Entry(cakesframe, font=(
    'candara', 18, 'bold'), bd=2, width=5, state=DISABLED, textvariable=e_BlackForest)
